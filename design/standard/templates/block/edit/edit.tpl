...
<div class="block-controls float-break">
    ...
    {if $is_custom|not}
     ...
     {/if}
     {def $view_array = ezini( $block.type, 'ViewList', 'block.ini' )}
     {if $view_array|count|gt(0)}
...
<div class="block-parameters float-break">
    ...
    {def $custom_attributes = ezini( $block.type, 'CustomAttributes', 'block.ini' )
             ...
             $custom_attributes_label = ezini( $block.type, 'CustomAttributeLabel', 'block.ini' )}
      {if $custom_attributes|count|gt(0)}
		<div class="blockname float-break">
        ...
        {foreach $custom_attributes as $custom_attrib}
            {if eq( $use_browse_mode[$custom_attrib], 'true' )}
                ...
                <input id="block-choose-source-{$block_id}" class="button block-control" name="CustomActionButton[{$attribute.id}_custom_attribute_browse-{$zone_id}-{$block_id}-{$custom_attrib}]" type="submit" value="{'Browse'|i18n( 'design/ezwebin/content/browse' )}" />
				</p>
            {else}
                <p><label>{if ne($custom_attributes_label[$custom_attrib],'')}{$custom_attributes_label[$custom_attrib]|wash}{else}{$custom_attrib}{/if}:</label>
